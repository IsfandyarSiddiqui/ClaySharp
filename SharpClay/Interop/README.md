# SharpClay Interop Layer

The `SharpClay.Interop` namespace provides low-level, zero-overhead, high-performance C# bindings to **Clay**—a single-header C library designed for fast 2D user interface layout. 
...
Instead of using classic, runtime-marshaled P/Invoke via `[DllImport]`, SharpClay utilizes modern .NET source-generated interop using `[LibraryImport]`.
...
C structures often utilize overlapping memory unions to save space or support polymorphism. C# does not support unions natively, so SharpClay emulates them using explicit structure layouts:
...
2. **Open Pass:** Signal the start of the layout pass:
   ...
   ```
3. **Declare Hierarchy:** Define UI elements in a tree structure. SharpClay facilitates this cleanly using C# disposable scopes (`using` statements):
   ```csharp
